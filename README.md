...
### Overview
This firmware reads a 6-axis IMU over SPI, fuses accelerometer and gyroscope data into a pitch and roll estimate using a complementary filter, and streams the result over USART for analysis. Input is taken from an RC remote and used to control the drone's attitude in flight by adjusting setpoints for the PID, and can also be used for tuning the PID controller in-flight. Every peripheral driver (clock tree, SPI, USART, timers, GPIO, sensors) was written from scratch using the respective documentation.

 **Current State:** *(Aug. 14 2026)* Drone is flying. Attitude control with RC input and basic PID tuning resulted in a stable hover when manually piloting. 
...
| timer.c | Configure TIM2 as 1kHz control loop & TIM5 as 1MHz RC clock |
...
| pid.c | Pitch/roll/yaw PID, motor mixing, output saturation handling, arm/disarm |
| rc.c | 8-channel PPM decoder over EXTI; reads controller input and maps to PID |
...
* **Flight test:** Achieved stable hover with smooth response to manual controls.
***
...
* Pitch, roll, yaw PID with motor mixing
#### In progress
